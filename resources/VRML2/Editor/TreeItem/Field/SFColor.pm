package VRML2::Editor::TreeItem::Field::SFColor;
use strict;
...
sub new {
...

    {
	    ...
	    $entry->set_usize(66, 0);
	    $entry->set_text($value->[2]);
		
		$entry->signal_connect('activate', sub { $this->on_enter(2); } );
		$entry->signal_connect('focus_out_event', sub { $this->on_enter(2, 1); } );
