package X3D::Gtk2::CellRendererNode;
use strict;
use warnings;
...
sub format_text {
	...
	my $referenceCount = $cell->{node}->getReferenceCount;
	...
	my $numParents = @{$cell->{node}->getValue->getParents};
	$string .= " ($numParents)";
...
sub RENDER {
	...
	$widget->get_style->paint_layout(
		...
		$cell_area->x() - 7 + $x_offset + icon_space + x_padding,
